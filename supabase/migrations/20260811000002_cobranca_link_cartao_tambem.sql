-- Migration 20260811000002 — checkout próprio (/pagar/:id) agora cobre Pix e
-- Cartão (tokenizado via pagar-cobranca-cartao), não só Pix. Estende o CASE
-- WHEN do lembrete de 7 dias antes (bloco D) que ainda mandava o aluno pro
-- link hospedado pelo Asaas sempre que a cobrança era CREDIT_CARD — mesmo
-- bug já corrigido em asaas-create-charge/index.ts e asaas-webhook/index.ts
-- nesta sessão, replicado aqui porque essa função SQL tinha a mesma lógica
-- duplicada (sem lib compartilhada entre Edge Functions e funções de banco).
    ...
    IF NOT rec.aluno_notificado_7d THEN
      ...
      VALUES (rec.aluno_user_id, rec.org_id,
        ...
        rec.descricao
          ...
          || ' — clique para renovar',
        'financeiro',
        CASE WHEN rec.forma_pagamento IN ('PIX', 'CREDIT_CARD')

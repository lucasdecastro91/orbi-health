-- ─────────────────────────────────────────────────────────────────────────────
-- Notificações de cobrança para o ALUNO + TREINADOR via cron
-- ─────────────────────────────────────────────────────────────────────────────

-- 1. Flags de deduplicação em cobrancas
ALTER TABLE public.cobrancas
  ADD COLUMN IF NOT EXISTS aluno_notificado_7d      boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS aluno_notificado_vencida boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS trainer_notificado_vencida boolean NOT NULL DEFAULT false;

-- 2. Substitui a função anterior com suporte completo a aluno + treinador
